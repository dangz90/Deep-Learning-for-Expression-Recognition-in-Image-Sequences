...
def save_as_numpy(path, where_to_save):
	for sub in os.listdir(path):
		for file in os.listdir(os.path.join(path, sub)):
			...
			files = os.path.join(path, 'EGG_numpy', file.replace('bdf','npy'))
			eggf = np.load(files)
			print(eggf.shape)

			raw = mne.io.read_raw_fif( os.path.join(path, sub, file) )
			...
			np.save(where_to_save +'/'+ file.split('.')[0] + '.npy', np.reshape(data, (data.shape[1], data.shape[2])))
...
if __name__ == "__main__":
	...
	"""
	data_path = os.path.join('..', '_Dataset')
	path_to_save = os.path.join('..', '_Dataset', 'EGG_numpy')

	save_as_numpy( data_path, path_to_save )
	# plot_channels(os.path.join(data_path, 'EGG', '1_posM_00001011_c1a1ki51.bdf'), 'plot')
