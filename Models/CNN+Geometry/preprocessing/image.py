...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        white_list_formats = {'png', 'jpg', 'jpeg', 'bmp', 'ppm', 'h5'}
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.h5 == True:
            batch_x = np.zeros((len(index_array),) + (136,), dtype=backend.floatx())
            # build batch of h5 files
            for i, j in enumerate(index_array):
                fname = self.filenames[j]
                f = h5py.File(os.path.join(self.directory, fname), 'r')
                x = np.array(f.get('geometry'))
                # Normalize
                new_origin = [112,135]
                new_x = x - new_origin
