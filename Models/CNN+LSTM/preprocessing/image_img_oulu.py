...
class DirectoryIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        def file_to_array(image_file, fname):
            img_path = os.path.join(image_file, fname)
        ...
        if self.h5 == True:
            ...
            for i, j in enumerate(index_array):
                ...
                file_path = self.directory.replace('consecutive', 'preface')
                file_list = [os.path.basename(fi) for fi in f]

                x = [np.array(file_to_array(file_path, os.path.join(os.path.dirname(fname), file.replace('.h5', '.jpeg')))) for file in file_list]
                # Normalize
                batch_x[i] = x
